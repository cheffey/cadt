...
import static org.chef.cadt.util.GeneralUtil.merge;
...
final class TestCase implements io.cucumber.plugin.event.TestCase {
    ...
    TestCase(
        ...
        this.caseFlow = new CaseFlow(merge(beforeHooks, testSteps, afterHooks));
        this.pickle = pickle;
    ...
    void run(EventBus bus) {
        ...
        caseFlow.currentStepIndex = 0;
        for (; caseFlow.currentStepIndex < caseFlow.allSteps.size(); caseFlow.currentStepIndex++) {
            io.cucumber.core.runner.TestStep step = caseFlow.allSteps.get(caseFlow.currentStepIndex);
            nextExecutionMode = run0(bus, state, step, nextExecutionMode);
        }
    ...
    private ExecutionMode run0(EventBus bus, TestCaseState state, io.cucumber.core.runner.TestStep step
